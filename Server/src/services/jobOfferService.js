...
const createJobOffer = async (
  ...
  try {
    ...
    return await jobOfferDao.createJobOffer(
      title,
      description,
      salary,
      location,
...
const updateJobOffer = async (
  ...
  try {
    return await jobOfferDao.updateJobOffer(
      id,
      title,
      description,
      salary,
...
const obtenerJobOffersFiltrados = async (filters) => {
  try {
    const jobOffers = await jobOfferDao.findFilteredJobOffers(filters);
    return jobOffers;
  } catch (error) {
    throw new Error(
      "Error in service while fetching filtered job offers: " + error.message
    );
  }
};
